...
public class BottomNavigationView extends RelativeLayout {
    ...
    private float textActiveSize = getResources().getDimension(R.dimen.bottom_navigation_text_size_active);
    private float textInactiveSize = getResources().getDimension(R.dimen.bottom_navigation_text_size_inactive);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int white = ContextCompat.getColor(context, R.color.background);
        ...
        if (isTablet) {
            ...
            items.setPadding(0, itemHeight / 3, 0, 0);
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            viewList.add(view);
            if (i == 0)
                icon.setImageResource(bottomNavigationItems.get(i).getActiveImageResource());
            else
                icon.setImageResource(bottomNavigationItems.get(i).getInactiveImageResource());
...
//                BottomNavigationUtils.imageColorChange(icon, itemActiveColorWithoutColoredBackground, itemInactiveColor);
     ...
     * @param activeImageResources   images resources for every item in the ViewPager adapter
     * @param inactiveImageResources images resources for every item in the ViewPager adapter
     */

    public void setUpWithViewPager(ViewPager pager, int[] colorResources, int[] activeImageResources, int[] inactiveImageResources) {
        this.mViewPager = pager;
        if (pager.getAdapter().getCount() != colorResources.length || pager.getAdapter().getCount() != activeImageResources.length)
        ...
        for (int i = 0; i < pager.getAdapter().getCount(); i++)
            addTab(new BottomNavigationItem(pager.getAdapter().getPageTitle(i).toString(), colorResources[i], activeImageResources[i], inactiveImageResources[i]));
